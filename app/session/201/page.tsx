...
import { tabs201, session201Meta } from '@/lib/content/session-201'
import { sessions } from '@/lib/content/shared'

const COLOUR = '#008278'
const SESSION_DATA = sessions.find((s) => s.id === '201')!

export default function Session201Page() {
  const { unlockedSessions, loading } = useAuth()
  const [activeTab, setActiveTab] = useState('field-guide')
  const [showModal, setShowModal] = useState(false)
  const { refreshUnlockedSessions } = useAuth()

  const unlocked = !loading && unlockedSessions.includes('201')
        ...
        <div style={{ background: '#111111', padding: '80px 24px', minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ maxWidth: 480, textAlign: 'center' }}>
            <div style={{
              ...
              height: 56,
              borderRadius: '50%',
              background: 'rgba(0,130,120,0.15)',
              border: '0.5px solid rgba(0,130,120,0.3)',
              display: 'flex',
              alignItems: 'center',
            ...
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', margin: '0 0 28px', lineHeight: 1.7 }}>
              Attend the 201 Walk session to unlock this content. You&apos;ll receive a code on the closing slide.
            </p>
            <button
  ...
  const currentTab = tabs201.find((t) => t.id === activeTab) ?? tabs201[0]
      ...
      <div style={{ background: '#111111', padding: '40px 24px 0' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>{session201Meta.subtitle}</span>
          </div>
          <h1 style={{ fontSize: 'clamp(22px, 4vw, 32px)', fontWeight: 500, color: '#F1F1F2', margin: '0 0 32px', lineHeight: 1.2 }}>
            Efficiency & automation with Cowork
          </h1>
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{
        ...
        Content coming after your 201 session
      </div>
